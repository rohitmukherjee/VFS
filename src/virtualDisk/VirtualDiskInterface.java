...
public interface VirtualDiskInterface {
	...
	/*
	 * This function will remove size number of bytes starting at location and ending
	 * at location+size.  This will set all of these bytes to null byte so that it 
	 * will not get confused with significant data in the future.
	 * 
	 ...
	 * 
	 * @throw IOException if there is problems accessing the meta-data of the file
	 * @return the sie of the current Virtual Hard Disk
	 */
	long getDiskSize() throws IOException;

	/*
	 * Queries the disk to find out how much free space is left on the virtual disk
	 * 
	...
	/*
	 * Queries the disk to find out how much space is currently allocated 
	 * on the virtual disk
	 * 
	 * @return the size of the unused memory on the disk.  Note it should hold that:
	 * 		getDiskSize() = getFreeSpace() + getAllocatedSpace();
	 */
	long getAllocatedSpace();

	/*
	 * Queries the disk to find out where the next unallocated block of memory is 
	 * 
	 * @return the location of the next free block to be written to
	 */
	long getNextFreeBlock();

	/*
	 * Writes data at the position.  This ensures that no other data will be overwritten
	 * 
	 * @param position the starting position of where the new data will be placed
	...
	/*
	 * Write data to the next available position and will fail if there is not enough 
	 * free space for data to be written
	 * 
	 * @return the location where data was written.
	 * @throws IOException if the is a problem accessing the .vdisk file
	 */
	long write(byte[] data) throws IOException;

	/*
	 * This reads the specified number of bytes from the location that is specified 
	 * 
